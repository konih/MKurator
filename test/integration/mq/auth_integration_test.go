...
func TestIntegration_GetChannelAuth_SslPeerMap(t *testing.T) {
	requireIntegration(t)
	ctx := testContext(t)
	channel := channelNameForTest(t.Name())
	sslPeerName := sslPeerNameForTest(t.Name())
	...
	authSpec := mqadmin.ChannelAuthSpec{
		...
		UserSource:  "MAP",
		McaUser:     "app",
		Description: "integration sslpeermap path",
	}
	t.Cleanup(func() {
		_ = c.DeleteChannelAuth(context.Background(), authSpec)
		_ = c.DeleteChannel(context.Background(), chSpec)
	...
	if !strings.EqualFold(state.SSLPeerName, sslPeerName) {
		t.Fatalf("sslPeerName = %q, want %q", state.SSLPeerName, sslPeerName)
	}
	if !strings.EqualFold(state.McaUser, "app") {
		t.Fatalf("mcaUser = %q, want app", state.McaUser)
	}
...
func TestIntegration_DeleteChannelAuth_SslPeerMap(t *testing.T) {
	requireIntegration(t)
	ctx := testContext(t)
	channel := channelNameForTest(t.Name())
	sslPeerName := sslPeerNameForTest(t.Name())
